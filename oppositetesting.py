...
def run_betting_script():
    ...
    with mss.mss() as sct:
        ...
        previous_game_result = "player"  # Initialize with a default choice
        ...
        while True:
            ...
            if not waiting_for_result and round_complete and not bets_open_detected:
                if detect_bets_open_text(sct):
                    # Place bet based on the previous game result
                    place_bet(previous_game_result)
                    ...
                    if prev_button_colors[button_name] is not None:
                        ...
                        if color_diff > 15 and (time.time() - last_change_time[button_name] > color_change_cooldown):
                            ...
                            if button_name == 'player':
                                previous_game_result = 'banker'
                            elif button_name == 'banker':
                                previous_game_result = 'player'
                            else:
                                previous_game_result = 'player'  # Default to player if tie
